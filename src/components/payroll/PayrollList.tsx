...
interface PayrollListProps {
  ...
  itemsPerPage?: number;
  fetchPayrolls: (page: number, pageSize: number) => Promise<{ data: Payroll[]; totalItems: number }>;
}
...
export default function PayrollList({ 
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetchPayrolls(currentPage, itemsPerPage);
    ...
    loadData();
  }, [currentPage, fetchPayrolls, itemsPerPage]);
  ...
  if (!payrolls.length) {
    return <div className="text-center py-10">No payroll records found.</div>;
  }
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {payrolls.map((p, i) => (
              <tr key={p.payrollId} className="hover:bg-gray-50 dark:hover:bg-gray-700">
