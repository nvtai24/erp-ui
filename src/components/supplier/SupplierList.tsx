...
interface SupplierListProps {
  ...
  itemsPerPage?: number;
  fetchSuppliers: (page: number, pageSize: number) => Promise<{ data: Supplier[]; totalItems: number }>;
}

export default function SupplierList({ onEdit, onDelete, itemsPerPage = 10, fetchSuppliers }: SupplierListProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetchSuppliers(currentPage, itemsPerPage);
    ...
    loadData();
  }, [currentPage, fetchSuppliers, itemsPerPage]);
  ...
  if (!suppliers.length) return <div className="text-center py-10">No Suppliers found.</div>;
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {suppliers.map((s, i) => (
              <tr key={s.supplierId} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{i + 1}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{s.supplierName}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{s.contact}</td>
